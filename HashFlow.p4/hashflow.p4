...
{
    modify_field(intrinsic_metadata.copy_to_cpu, 1);
...
register ancillary_table_key
{
    width: 8;
    instance_count: ANCILLARY_TABLE_SIZE;
}

blackbox stateful_alu update_ancillary_table_key
{
    reg: ancillary_table_key;
    condition_lo: register_lo == 0;
    condition_hi: register_lo == measurement_meta.digest;

    update_lo_1_value: measurement_meta.digest;

    output_value: predicate;
    output_dst: measurement_meta.ancillary_table_predicate;
}

action update_ancillary_table_key_action()
{
    update_ancillary_table_key.execute_stateful_alu_from_hash(hash_4);
}

table update_ancillary_table_key_t
{
    reads {
        measurement_meta.promotion: exact;
    ...
    actions {
        update_ancillary_table_key_action;
        nop;
    }
    default_action: nop;
}

register ancillary_table_value
{
    width: 8;
    instance_count: ANCILLARY_TABLE_SIZE;
}

blackbox stateful_alu update_ancillary_table_value
{
    reg: ancillary_table_value;
    condition_lo: measurement_meta.ancillary_table_predicate == 0x1;
    ...
    output_predicate: measurement_meta.ancillary_table_predicate == 0x4;
    output_value: register_lo;
    output_dst: measurement_meta.ac_flow_count;
}

action update_ancillary_table_value_action()
{
    update_ancillary_table_value.execute_stateful_alu_from_hash(hash_4);
}

table update_ancillary_table_value_t
{
    reads {
        measurement_meta.promotion: exact;
        measurement_meta.main_table_1_predicate: exact;
...
{
    reads {
        measurement_meta.ancillary_table_predicate: exact;
    }
    actions {
